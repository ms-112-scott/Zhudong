          ...
          {legendData.map((item, index) => {
            ...
            return (
              <div
                ...
                className={`flex items-center gap-3 cursor-pointer transition-all duration-300 group ${isDimmed ? 'opacity-30 hover:opacity-100' : 'opacity-100'
                  }`}
              >
                {/* 顏色圓點 */}
                <span
                  className={`w-3 h-3 rounded-full shadow-sm transition-transform duration-300 ${activeGroup === item.label ? 'scale-125 ring-2 ring-white/30' : 'group-hover:scale-110'
                    }`}
                  style={{ backgroundColor: item.color }}
                />

                {/* 文字標籤 */}
                <span className={`text-xs font-medium tracking-wide min-w-max transition-colors ${activeGroup === item.label ? 'text-white font-bold' : 'text-gray-300 group-hover:text-white'
                  }`}>
                  {item.label}
                </span>
              </div>
